...
import { Send as SendIcon, RefreshCcw, Pause, Play, TrendingUp, XCircle, Search, Bot, ShieldAlert } from 'lucide-react';
import { telegramApi, campaignApi } from '../../../../packages/ui/src/services/api';
...
export default memo(function TelegramDashboard() {
  const queryClient = useQueryClient();
  const counters = useAppStore((s) => s.counters);
  const [search, setSearch] = useState('');

  const { data: campaigns } = useQuery({
    queryKey: ['campaigns', 'telegram'],
    queryFn: () => campaignApi.list({ channel: 'telegram' }),
    refetchInterval: 30_000,
  });

  const { data: providerStats } = useQuery({
    queryKey: ['telegram-providers'],
    queryFn: telegramApi.getProviderStats,
  ...
  const retryMutation = useMutation({
    mutationFn: (id: string) => telegramApi.retryFailed(id),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2.5 rounded-lg bg-sky-500"><SendIcon className="w-6 h-6 text-white" /></div>
        <div>
          <h1 className="text-2xl font-bold">Telegram</h1>
          <p className="text-gray-400">Bot API with inline keyboards & rich media</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <StatsCard title="Sent Today" value={counters.telegram?.sent || 0} icon={TrendingUp} color="bg-emerald-600" />
        <StatsCard title="Failed Today" value={counters.telegram?.failed || 0} icon={XCircle} color="bg-red-600" />
        <StatsCard title="Blocked" value={0} icon={ShieldAlert} color="bg-orange-600" />
        <StatsCard title="In Queue" value={counters.telegram?.queued || 0} icon={Bot} color="bg-sky-600" />
      </div>
