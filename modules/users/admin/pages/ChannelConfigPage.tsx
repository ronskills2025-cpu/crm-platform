...
import {
  AlertTriangle, CheckCircle2, PauseCircle, PlayCircle,
  RefreshCw, Save, Send, Trash2, Webhook,
} from 'lucide-react';
...
const defaultsByChannel: Record<Channel, FormState> = {
  whatsapp: {
    name: 'meta', display_name: 'Meta Primary', priority: 10, rate_per_sec: 80,
    cost_per_msg: 0, daily_limit: 0, is_active: true,
    phoneNumberId: '', accessToken: '', apiVersion: 'v21.0',
    webhookUrl: DEFAULT_WEBHOOK_URL, verifyToken: DEFAULT_VERIFY_TOKEN,
    providerType: 'fast2sms', apiKey: '', authKey: '', senderId: '', sender: '',
    region: 'IN', apiUrl: '', botToken: '', webhookSecret: '',
    pageAccessToken: '', pageId: '', appSecret: '', fbVerifyToken: '',
    fromEmail: '', fromName: '',
    host: 'smtp.gmail.com', port: 587, user: '', pass: '', secure: false,
  },
  sms: {
    name: 'fast2sms', display_name: 'Fast2SMS', priority: 10, rate_per_sec: 50,
    cost_per_msg: 0, daily_limit: 0, is_active: true,
    phoneNumberId: '', accessToken: '', apiVersion: 'v21.0',
  ...
  email: {
    name: 'smtp_primary', display_name: 'SMTP Primary', priority: 10, rate_per_sec: 14,
    cost_per_msg: 0, daily_limit: 0, is_active: true,
    phoneNumberId: '', accessToken: '', apiVersion: 'v21.0',
    webhookUrl: '', verifyToken: '',
    providerType: 'smtp', apiKey: '', authKey: '', senderId: '', sender: '',
    region: 'IN', apiUrl: '', botToken: '', webhookSecret: '',
    pageAccessToken: '', pageId: '', appSecret: '', fbVerifyToken: '',
    fromEmail: '', fromName: '',
    host: 'smtp.gmail.com', port: 587, user: '', pass: '', secure: false,
  ...
  telegram: {
    name: 'telegram_bot', display_name: 'Telegram Bot', priority: 10, rate_per_sec: 30,
    cost_per_msg: 0, daily_limit: 0, is_active: true,
    phoneNumberId: '', accessToken: '', apiVersion: 'v21.0',
    webhookUrl: '', verifyToken: '',
    providerType: 'telegram', apiKey: '', authKey: '', senderId: '', sender: '',
    region: '', apiUrl: '', botToken: '', webhookSecret: '',
    pageAccessToken: '', pageId: '', appSecret: '', fbVerifyToken: '',
    fromEmail: '', fromName: '',
    host: '', port: 587, user: '', pass: '', secure: false,
  },
  messenger: {
    name: 'fb_page', display_name: 'Facebook Page', priority: 10, rate_per_sec: 200,
    cost_per_msg: 0, daily_limit: 0, is_active: true,
    phoneNumberId: '', accessToken: '', apiVersion: 'v21.0',
    webhookUrl: '', verifyToken: '',
    providerType: 'messenger', apiKey: '', authKey: '', senderId: '', sender: '',
    region: '', apiUrl: '', botToken: '', webhookSecret: '',
    pageAccessToken: '', pageId: '', appSecret: '', fbVerifyToken: 'fb_verify_token',
    fromEmail: '', fromName: '',
    host: '', port: 587, user: '', pass: '', secure: false,
  },
};
...
function statusClass(status: ProviderStatus): string {
  if (status === 'connected') return 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-300 dark:border-emerald-500/20';
  if (status === 'failed') return 'bg-red-100 dark:bg-red-500/10 text-red-700 dark:text-red-400 border-red-300 dark:border-red-500/20';
  if (status === 'paused') return 'bg-yellow-100 dark:bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 border-yellow-300 dark:border-yellow-500/20';
  return 'bg-gray-100 dark:bg-gray-500/10 text-gray-600 dark:text-gray-300 border-gray-300 dark:border-gray-500/20';
}
...
export default function ChannelConfigPage() {
  ...
  function loadProvider(provider: ProviderRecord) {
    ...
    setForm((prev) => ({
      ...
      botToken: '',
      webhookSecret: toInput(c.webhookSecret, true),
      pageAccessToken: '',
      pageId: toInput(c.pageId, true),
      appSecret: '',
      fbVerifyToken: toInput(c.verifyToken, true) || 'fb_verify_token',
    }));
  }
  ...
  function buildPayload() {
    ...
    return {
      ...
      credentials: compact({
        ...
        verifyToken: form.fbVerifyToken.trim() || 'fb_verify_token',
      }),
    };
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">Configure WhatsApp, SMS, Email, Telegram, and Messenger directly from admin panel.</p>
        </div>
        <button onClick={refreshAll}
          className="px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 inline-flex items-center gap-2">
          <RefreshCw className="w-4 h-4" /> Refresh
        </button>
      </div>

      <div className="flex gap-2 flex-wrap">
        {(['whatsapp', 'sms', 'email', 'telegram', 'messenger'] as Channel[]).map((item) => (
          <button key={item} onClick={() => changeChannel(item)}
        ...
        <form onSubmit={submitProvider} className="card border border-gray-200 dark:border-gray-800 space-y-4">
          ...
          {channel === 'whatsapp' && (
            <div className="space-y-3 border-t border-gray-200 dark:border-gray-800 pt-4">
              ...
              <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-3 space-y-2">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">These values are fixed and used automatically.</p>
              </div>
            </div>
          )}
          ...
          {channel === 'sms' && (
            <div className="space-y-3 border-t border-gray-200 dark:border-gray-800 pt-4">
              ...
              <label className="text-sm space-y-1 block"><span>Auth Token (if needed)</span><input type="password" placeholder={editingId ? 'Leave blank to keep existing token' : ''} className={fieldClass()} value={form.authKey} onChange={(e) => setForm({ ...form, authKey: e.target.value })} /></label>
          ...
          {channel === 'telegram' && (
            <div className="space-y-3 border-t border-gray-200 dark:border-gray-800 pt-4">
              <label className="text-sm space-y-1 block"><span>Bot Token</span><input type="password" placeholder={editingId ? 'Leave blank to keep existing token' : 'e.g. 123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11'} className={fieldClass()} value={form.botToken} onChange={(e) => setForm({ ...form, botToken: e.target.value })} /></label>
              <label className="text-sm space-y-1 block"><span>Webhook Secret (optional)</span><input className={fieldClass()} value={form.webhookSecret} onChange={(e) => setForm({ ...form, webhookSecret: e.target.value })} /></label>

              <div className="rounded-xl border border-sky-500/20 bg-sky-500/5 p-3 space-y-2">
                <p className="text-sm font-medium text-sky-600 dark:text-sky-300">Telegram Bot Setup</p>
                <ol className="text-xs text-gray-500 dark:text-gray-400 space-y-1 list-decimal list-inside">
                  ...
                  <li>Save the config, then click "Validate" to verify</li>
                  <li>Set your webhook URL to receive incoming messages</li>
                </ol>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Rate limit: 30 messages/sec per bot (Telegram enforced). Free API, no per-message cost.</p>
              </div>
            </div>
          )}
          ...
          {channel === 'messenger' && (
            <div className="space-y-3 border-t border-gray-200 dark:border-gray-800 pt-4">
              <label className="text-sm space-y-1 block"><span>Page Access Token</span><input type="password" placeholder={editingId ? 'Leave blank to keep existing token' : 'Paste your long-lived Page Access Token'} className={fieldClass()} value={form.pageAccessToken} onChange={(e) => setForm({ ...form, pageAccessToken: e.target.value })} /></label>
              <label className="text-sm space-y-1 block"><span>Page ID</span><input className={fieldClass()} placeholder="e.g. 123456789012345" value={form.pageId} onChange={(e) => setForm({ ...form, pageId: e.target.value })} /></label>
              <label className="text-sm space-y-1 block"><span>App Secret</span><input type="password" placeholder={editingId ? 'Leave blank to keep existing secret' : 'From Facebook App Dashboard → Settings → Basic'} className={fieldClass()} value={form.appSecret} onChange={(e) => setForm({ ...form, appSecret: e.target.value })} /></label>
              <label className="text-sm space-y-1 block"><span>Verify Token</span><input className={fieldClass()} value={form.fbVerifyToken} onChange={(e) => setForm({ ...form, fbVerifyToken: e.target.value })} /></label>

              <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-3 space-y-2">
                <p className="text-sm font-medium text-blue-600 dark:text-blue-300">Facebook Messenger Setup</p>
                <ol className="text-xs text-gray-500 dark:text-gray-400 space-y-1 list-decimal list-inside">
                  <li>Go to developers.facebook.com → Create/select an app</li>
                  <li>Add the Messenger product to your app</li>
                  <li>Generate a Page Access Token (link your Page)</li>
                  <li>Paste the token and Page ID above</li>
                  <li>Set your webhook URL to: <code className="bg-gray-100 dark:bg-gray-950 px-1 rounded">https://yourdomain.com/api/messenger/webhook</code></li>
                  <li>Subscribe to: messages, messaging_postbacks, messaging_optins</li>
                </ol>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Rate limit: 200 calls/hour per page (Messenger Platform). Free to send within 24h window.</p>
              </div>
            </div>
          )}
          ...
          <button disabled={saving} className="w-full rounded-lg bg-brand-600 hover:bg-brand-500 disabled:opacity-60 text-white px-4 py-2.5 font-medium inline-flex items-center justify-center gap-2">
              ...
              {providers.map((provider) => (
                <div key={provider.id} className="rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/60 p-4">
                  ...
                  <div className="flex gap-2 flex-wrap mt-3">
                    ...
                    {channel === 'whatsapp' && (
                      <button onClick={() => registerWebhook(provider)} className="px-3 py-1.5 rounded-lg bg-cyan-600/15 text-cyan-700 dark:text-cyan-300 hover:bg-cyan-600/25 text-sm inline-flex items-center gap-1"><Webhook className="w-4 h-4" /> Register Webhook</button>
                    )}
          ...
          <div className="space-y-3">
            {groupedErrors.map((item) => (
              <div key={item.id} className="rounded-xl border border-red-500/20 bg-red-500/5 p-4 flex items-center justify-between gap-3 flex-wrap">
                ...
                <button onClick={() => retryCampaign(item.campaign_id)} className="px-3 py-2 rounded-lg bg-brand-600 hover:bg-brand-500 text-white text-sm">Retry & Resume</button>
              </div>
            ))}
          </div>
        )}
