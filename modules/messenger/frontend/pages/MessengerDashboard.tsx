...
import { MessageCircle, RefreshCcw, Pause, Play, TrendingUp, XCircle, Search, ShieldAlert, Users } from 'lucide-react';
import { messengerApi, campaignApi } from '../../../../packages/ui/src/services/api';
...
export default memo(function MessengerDashboard() {
  const queryClient = useQueryClient();
  const counters = useAppStore((s) => s.counters);
  const [search, setSearch] = useState('');

  const { data: campaigns } = useQuery({
    queryKey: ['campaigns', 'messenger'],
    queryFn: () => campaignApi.list({ channel: 'messenger' }),
    refetchInterval: 30_000,
  });

  const { data: providerStats } = useQuery({
    queryKey: ['messenger-providers'],
    queryFn: messengerApi.getProviderStats,
  ...
  const retryMutation = useMutation({
    mutationFn: (id: string) => messengerApi.retryFailed(id),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2.5 rounded-lg bg-blue-600"><MessageCircle className="w-6 h-6 text-white" /></div>
        <div>
          <h1 className="text-2xl font-bold">Messenger</h1>
          <p className="text-gray-400">Facebook Messenger with templates, buttons & quick replies</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <StatsCard title="Sent Today" value={counters.messenger?.sent || 0} icon={TrendingUp} color="bg-emerald-600" />
        <StatsCard title="Failed Today" value={counters.messenger?.failed || 0} icon={XCircle} color="bg-red-600" />
        <StatsCard title="Blocked" value={0} icon={ShieldAlert} color="bg-orange-600" />
        <StatsCard title="In Queue" value={counters.messenger?.queued || 0} icon={Users} color="bg-blue-600" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div key={c.id as string} className="flex items-center justify-between p-3 rounded-lg bg-gray-100 dark:bg-gray-800/50 hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors">
                <div>
                  ...
                  <div className="flex items-center gap-3 text-xs text-gray-400 mt-1">
                    ...
                    <span>Total: {formatNumber((c.total_contacts as number) || 0)}</span>
                  </div>
                </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="card">
          <h2 className="font-semibold mb-4">Provider Performance (24h)</h2>
          <div className="space-y-3">
            {providerStats?.stats?.length === 0 && <p className="text-sm text-gray-500 text-center py-4">No data yet</p>}
            {providerStats?.stats?.map((s: Record<string, unknown>, i: number) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-gray-100 dark:bg-gray-800/50">
                <div><p className="font-medium capitalize">{s.provider_used as string}</p><p className="text-xs text-gray-400">Status: {s.status as string}</p></div>
