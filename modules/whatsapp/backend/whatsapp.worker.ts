...
const log = createLogger('worker:whatsapp');
...
const worker = new Worker<BatchJobData>(
  ...
  async (job: Job<BatchJobData>) => {
    ...
    for (const msg of messages) {
      const result = await executeWithFailover(
        'whatsapp',
        msg.provider_chain,
        { recipient: msg.phone, campaign_id, message: msg.message, template_id: msg.template_id, template_params: msg.template_params },
        async (provider, payload) =>
          WhatsAppService.sendViaProvider(
            provider,
            payload.recipient as string,
            payload.message as string,
            payload.template_id as string | undefined,
            payload.template_params as string[] | undefined
          )
      );

      if (result.success) {
        await query(
          `UPDATE whatsapp_messages SET status = 'sent', provider_used = $1, provider_msg_id = $2, cost = $3, sent_at = NOW(), attempts = attempts + 1 WHERE id = $4`,
          [result.providerUsed, result.providerMsgId, result.cost || 0, msg.id]
        );
        sentCount++;
        await incrCounter(`stats:whatsapp:sent:${todayKey()}`);
      } else {
        const res = await query<{ attempts: number }>('SELECT attempts FROM whatsapp_messages WHERE id = $1', [msg.id]);
        ...
        if (needsManualRecovery || attemptCount >= config.maxRetries) {
          await query(
            `UPDATE whatsapp_messages SET status = 'failed', error_message = $1, attempts = $2 WHERE id = $3`,
            [result.error, attemptCount, msg.id]
          );
          await storeFailedMessage('whatsapp', campaign_id, msg.id, msg.phone, msg.message, msg.provider_chain, result.error || '', attemptCount);
          ...
          await pauseCampaignForRecovery(
            campaign_id,
            'whatsapp',
            needsManualRecovery ? 'WhatsApp provider configuration needs attention' : 'WhatsApp message failed after retries',
            { provider: result.providerUsed, message_id: msg.id, recipient: msg.phone }
          );
          failCount++;
          await incrCounter(`stats:whatsapp:failed:${todayKey()}`);
          publishEvent('whatsapp:message_failed', { campaign_id, message_id: msg.id, recipient: msg.phone, error: result.error });
        } else {
          const delayMs = Math.min(1000 * 2 ** attemptCount, 60_000);
          await query(
            `UPDATE whatsapp_messages SET status = 'queued', error_message = $1, attempts = $2 WHERE id = $3`,
            [result.error, attemptCount, msg.id]
          );
          ...
          publishEvent('whatsapp:retry_scheduled', {
            campaign_id,
            message_id: msg.id,
            recipient: msg.phone,
            retryInMs: delayMs,
    ...
    if (failCount > 0 && sentCount === 0) {
      ...
      if (c && c.total_contacts > 0 && c.failed_count / c.total_contacts > 0.5) {
        ...
        publishEvent('campaign:paused', { campaign_id, channel: 'whatsapp', reason: 'High failure rate' });
  ...
  { connection: redis, concurrency: config.whatsapp.concurrency }
);
...
async function checkCampaignCompletion(campaignId: string) {
  ...
  if (c && c.sent_count + c.failed_count >= c.total_contacts) {
    ...
    publishEvent('campaign:completed', { campaign_id: campaignId, channel: 'whatsapp' });
  }
}
...
log.info('WhatsApp worker started');
