...
import { MessageSquare, RefreshCcw, Pause, Play, TrendingUp, XCircle, Search } from 'lucide-react';
import { whatsappApi, campaignApi } from '../../../../packages/ui/src/services/api';
...
export default memo(function WhatsAppDashboard() {
  ...
  const { data: campaigns } = useQuery({
    ...
    queryFn: () => campaignApi.list({ channel: 'whatsapp' }),
    staleTime: 30_000,
  ...
  const { data: providerStats } = useQuery({
    ...
    queryFn: whatsappApi.getProviderStats,
    staleTime: 30_000,
    refetchInterval: 60_000,
  });
  ...
  const retryMutation = useMutation({
    mutationFn: (id: string) => whatsappApi.retryFailed(id),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2.5 rounded-lg bg-whatsapp"><MessageSquare className="w-6 h-6 text-white" /></div>
        <div>
          <h1 className="text-2xl font-bold">WhatsApp</h1>
          <p className="text-gray-400">Meta Cloud API with template support</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatsCard title="Sent Today" value={counters.whatsapp.sent} icon={TrendingUp} color="bg-emerald-600" />
        <StatsCard title="Failed Today" value={counters.whatsapp.failed} icon={XCircle} color="bg-red-600" />
        <StatsCard title="In Queue" value={counters.whatsapp.queued} icon={MessageSquare} color="bg-yellow-600" />
      </div>
