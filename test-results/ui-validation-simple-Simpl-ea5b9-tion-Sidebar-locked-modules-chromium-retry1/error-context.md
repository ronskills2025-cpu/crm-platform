...
- Name: ui-validation-simple.spec.ts >> Simple UI Validation >> Sidebar locked modules
- Location: e2e\ui-validation-simple.spec.ts:90:7
...
Locator: locator('nav a[href="/wa-chat"]')
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('nav a[href="/wa-chat"]')
...
> 101 |     await expect(waChatLink).toBeVisible();
  ...
  159 |     if (hasActivity) console.log('✅ Activity section found');
  160 |     
  161 |     // Check that no hardcoded dummy data is present
  162 |     const pageContent = await page.textContent('body');
  163 |     const hasDummyData = pageContent?.includes('12,458') || pageContent?.includes('48.2K');
  164 |     
  165 |     if (!hasDummyData) {
  166 |       console.log('✅ No hardcoded dummy data found');
  167 |     } else {
  168 |       console.log('⚠️ Some dummy data may still be present');
  169 |     }
  170 |   });
  171 | });
  172 | 
